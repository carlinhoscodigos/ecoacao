...
export function AppProvider({ children }) {
  ...
  const refreshApi = useCallback(async () => {
    ...
    const [meRes, rankRes, logsRes, stRes] = responses;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.error('Falha ao inicializar sessao:', err);
        if (hasApiConfigured()) resetApiState();
      } finally {
  ...
  }, [refreshApi, refreshLocal, resetApiState]);
  ...
  async function loginUserApi({ email, password }) {
    ...
    try {
      ...
      console.error('Falha ao chamar login da API:', err);
      return {
        ok: false,
        error: 'Nao foi possivel conectar ao backend. Verifique se a API esta online.',
      };
    }
    ...
    if (!refreshed) {
      ...
      return {
        ok: false,
        error: 'Login recebido, mas nao foi possivel carregar seus dados.',
      };
    }
